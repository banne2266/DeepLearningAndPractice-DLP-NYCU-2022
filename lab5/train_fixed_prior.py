...
### call: loss, mse, kld = train(seq, cond, modules, optimizer, kl_anneal, args)
...
def main():
    ...
    # --------- load a dataset ------------------------------------
    train_data = bair_robot_pushing_dataset(args, 'train', seq_len=args.n_past + args.n_future)
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        if epoch % 5 == 0:
            psnr_list = []
            for _ in range(len(validate_data) // args.batch_size):#len(validate_data) // args.batch_size
                try:
    ...
    plt.plot(msess, label="mse losses")
    plt.plot(klds, label="KLD losses")
    plt.ylabel("Loss")
    plt.xlabel("Epoches")
    plt.legend()
    plt.show()

    plt.plot(tfrs, label="Teacher Ratio")
    plt.plot(betas, label="KLD weight")
    plt.ylabel("Ratio")
    plt.xlabel("Epoches")
    plt.legend()
    plt.show()

    plt.plot(psnrs, label="Average PSNRs")
    plt.ylabel("PSNRs")
    plt.xlabel("Epoches (validated in every 5 epoches)")
    plt.legend()
    plt.show()
